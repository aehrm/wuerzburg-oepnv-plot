...
@Component({
  ...
  template: `
    ...
    <div>
      <p>
        Von
        <input type="time" [(ngModel)]="startTime" (input)="onUpdateDates()" />
        bis
        <input type="time" [(ngModel)]="endTime" (change)="onUpdateDates()" />;
        Tag
        <input type="date" [(ngModel)]="searchDay" (change)="onUpdateDates()" />
      </p>
      <div class="search-select-container">
        <search-select
          placeholder="Suche nach Haltestelle..."
          [fetchOptions]="searchStops"
...
export class StopLocationSearchComponent {
  ...
  onUpdateDates(): void {
    ...
    const newStartDate = DateTime.fromISO(this.searchDay, {
      zone: "Europe/Berlin",
    }).set({
    ...
    const newEndDate = DateTime.fromISO(this.searchDay, {
      zone: "Europe/Berlin",
    }).set({
