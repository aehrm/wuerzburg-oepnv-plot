...
export class ApiService {
  ...
  async searchStops(searchTerm: string): Promise<StopLocation[]> {
    const baseUrl =
      "https://fahrtauskunft.avv-augsburg.de/efa/XML_STOPFINDER_REQUEST";
    ...
    const locations: StopLocation[] = data.locations
      .filter((location: any) => {
  ...
  async getStopsAtLocation(
    ...
    const getStopLocation = function (stopLocationObj: any): StopLocation {
      if (stopLocationObj.type === "stop") {
        return {
          name: stopLocationObj.name,
          shortName: stopLocationObj.disassembledName,
          id: stopLocationObj.id,
        };
      } else {
    ...
    const toStop = function (loc: any, trip: Trip, tripIndex: number): Stop {
      ...
      return {
        ...
        tripIndex: tripIndex,
        arrivalTime:
          loc.arrivalTimePlanned !== undefined
            ? DateTime.fromISO(loc.arrivalTimePlanned)
            : undefined,
        departureTime:
          loc.departureTimePlanned !== undefined
            ? DateTime.fromISO(loc.departureTimePlanned)
            : undefined,
      };
    };

    const stops: Stop[] = (<any[]>data.stopEvents)
      .map((stopEvent: any) => {
        assert(getStopLocation(stopEvent.location).id === stopLocation.id);
        const line: Line = {
          id: stopEvent.transportation.id,
          name: stopEvent.transportation.name,
          description: stopEvent.transportation.description,
          destinationDesc: stopEvent.transportation.destination.name,
        };
        const trip: Trip = new Trip({
          line: line,
          tripCode: stopEvent.transportation.properties.tripCode,
        });
        ...
        const mainStop: Stop = {
          ...
          location: stopLocation,
          trip: trip,
          tripIndex: mainTripIndex,
        };

        const previousLocations: any[] =
          <any[] | undefined>stopEvent.previousLocations ?? [];
        const onwardLocations: any[] =
          <any[] | undefined>stopEvent.onwardLocations ?? [];

        const previousStops: Stop[] = previousLocations
          .map((locObj, i) => toStop(locObj, trip, i))
          .sort(
            (a, b) => a.departureTime!.toMillis() - b.departureTime!.toMillis(),
          );
        const onwardStops: Stop[] = onwardLocations
          .map((locObj, i) => toStop(locObj, trip, i + mainTripIndex + 1))
          .sort(
            (a, b) => a.arrivalTime!.toMillis() - b.arrivalTime!.toMillis(),
          );
      ...
      .filter(
        (stop) =>
          (stop.departureTime ?? endDate) <= endDate &&
          (stop.arrivalTime ?? endDate) <= endDate,
      );
