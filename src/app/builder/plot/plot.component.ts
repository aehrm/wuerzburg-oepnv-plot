...
export class TimetablePlotComponent {
  ...
  layout() {
    ...
    const stopClusters = new Map(Object.entries({
      "sanderring": new Set(["de:09663:348", "de:09663:346"]),
      "jupo": new Set(["de:09663:208", "de:09663:56"]),
      "hbf": new Set(["de:09663:177", "de:09663:68", "de:09663:170", "de:09663:179", "de:09663:75"]),
    }));

    const convertStopId = (stopId: string) => {
      const matchingClusters = [...stopClusters.entries()].filter(([_, cluster]) => cluster.has(stopId))
    ...
    [...generateCoordinates()].forEach(([id, x]) => this.stopPositions.set(id, x));
    this.updatePlot();
  }
