        ...
        <ng-template #itemTemplate let-item>
          <div class="trip-table-item-container">
            ...
            >
              ...
              nach
              {{
                item.stop.trip.stops.at(-1)!.location.shortName ??
                  item.stop.trip.stops.at(-1)!.location.name
              }}
            </div>
...
export class TripSelectorComponent {
  ...
  tableGroups = computed(() => {
    return [...this.stops().entries()]
      .sort(([a, ignore1], [b, ignore2]) => {
        return (a.name + a.destinationDesc).localeCompare(
          b.name + b.destinationDesc,
        );
      })
      .map(([line, stops]) => {
  ...
  async loadStops(): Promise<void> {
    ...
    try {
      ...
      const stops = await this.apiService.getStopsAtLocation(
        this.stopLocation(),
        this.startDate()!,
        this.endDate()!,
      );
      ...
      this.stops.set(
        new Map(
          [...stopsGroupedByLine.entries()].map((x) => [
            idToLine.get(x[0])!,
            x[1],
          ]),
        ),
  ...
  addTripToEditor(stop: Stop): void {
    ...
    const tripsOfSameLine = this.editorService
      .items()
      .filter((x) => lineEquality(x.trip));
    if (tripsOfSameLine.length > 0) {
      selectedStopsOfTrip = tripsOfSameLine[0].selectedStopIndices;
    } else {
      selectedStopsOfTrip = new Set(
        [...Array(trip.stops.length).keys()].filter((i) => i >= stop.tripIndex),
      );
    }
